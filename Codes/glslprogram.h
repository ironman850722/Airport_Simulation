...
bool Frozen;
bool Light0On, Light1On, Light2On, Light3On;
float MoveDistance;
GLuint	SphereList;
GLuint	ObjectList;

int		Distort;				// Distort the shape
...
Display()
{
	...
	//SetSpotLight(GL_LIGHT1, 5 * cos(Time * 2 * M_PI), 0, 5 * sin(Time * 2 * M_PI), -5 * cos(Time * 2 * M_PI), 0, -5 * sin(Time * 2 * M_PI), 1, 0, 0);
	//SetSpotLight(GL_LIGHT2, 0, 0, MoveDistance - 2, 0, 0, -1, 0, 0, 1);
	SetPointLight(GL_LIGHT3, 0, 3, 3, 0, 1, 0);
	...
	//glLightModelfv(GL_LIGHT_MODEL_AMBIENT, MulArray3(0.3f, White));
	...
	SetMaterial(1, 1, 0, 30); // yellow
	glCallList(SphereList);
	glDisable(GL_TEXTURE_2D);
	glPopMatrix();

	glPushMatrix();
	glTranslatef(0, 2 * sin(Time * 2 * M_PI), 2);
	...
	// Red light
	glPushMatrix();
	glTranslatef(5 * cos(Time * 2 * M_PI), 0, 5 * sin(Time * 2 * M_PI));
	glScalef(0.1, 0.1, 0.1);
	...
	// Blue light
	glPushMatrix();
	glTranslatef(0, 0, MoveDistance);
	glScalef(0.1, 0.1, 0.1);
	//SetMaterial(0, 0, 1, 30);
	glColor3f(0, 0, 1);
	glCallList(SphereList);
	glPopMatrix();
...
InitGraphics( )
{
	...
	//Texture world map to object
	unsigned char* Texture;
	int width, height;
	Texture = BmpToTexture("fire.bmp", &width, &height);
...
Reset( )
{
	...
	Xrot = Yrot = 0.;
	Distort = 0;
	Light0On = Light1On = Light2On = Light3On = true;
	Frozen = false;
}
...
OsuSphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
											...
											// ilng=NumLngs-1, lng=+M_PI are the same meridian
		{
			...
			if (Distort) {
				//p->s = (lng + M_PI) / (2. * M_PI) + (cos(Time * 2. * M_PI + M_PI / 2.) *lat / 4.);
				p->s = (lng + M_PI) / (2. * M_PI) + (sin(Time * 2 *M_PI + M_PI / 2) * lat / 4.);
				p->t = ((lat + M_PI / 2.) / M_PI);
